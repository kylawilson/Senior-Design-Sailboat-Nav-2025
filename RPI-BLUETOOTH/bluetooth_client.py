...
DEPTH_SVC_UUID =     '36523c64-9a13-4742-89d8-91c9db2374c0'
...
def depth_start_notify_cb():
    print('Depth notifications enabled')


def depth_changed_cb(iface, changed_props, invalidated_props):
    ...
    print('New Depth Measurement')

    #this just parses the message recived, we will have to parse differently
    flags = value[0]
...
def start_client():
    ...
    # Characteristic.
    depth_prop_iface = dbus.Interface(depth_chrc[0], DBUS_PROP_IFACE)
    depth_prop_iface.connect_to_signal("PropertiesChanged",
                                          depth_changed_cb)

    # Subscribe to Heart Rate Measurement notifications.
    depth_chrc[0].StartNotify(reply_handler=depth_start_notify_cb,
                                 error_handler=generic_error_cb,
                                 dbus_interface=GATT_CHRC_IFACE)
...
def process_depth_service(service_path):
    ...
    if uuid != DEPTH_SVC_UUID:
        print('Service is not a Depth Service: ' + uuid)
        return False
...
def interfaces_removed_cb(object_path, interfaces):
    if not depth_service:
        return

    if object_path == depth_service[2]:
