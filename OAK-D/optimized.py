...
# Set stereo depth profile (choosing a less dense option for faster processing)
stereo.setDefaultProfilePreset(dai.node.StereoDepth.PresetMode.HIGH_DENSITY)  # Change to MEDIUM_DENSITY for speed
stereo.setLeftRightCheck(True)
stereo.setSubpixel(False)  # Disable subpixel for faster processing
...
with dai.Device(pipeline) as device:
    ...
    with open("roi_distances.txt", "a") as file:  # Open the file in append mode
        ...
        while True:
            ...
            depthFrame = cv2.medianBlur(depthFrame, 3)  # Reduced blur kernel size
            ...
            if video.has():
                ...
                if np.all(depth_downscaled == 0):
                    min_depth = 0  # Default minimum depth value when all elements are zero
                else:
                ...
                # Overlay ROIs on both normal video feed and depth heatmap
