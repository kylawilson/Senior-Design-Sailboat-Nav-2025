...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if current_time - last_capture_time >= capture_interval:
            ...
            image_filename = os.path.join(save_folder, f"frame_{timestamp}.jpg")
            serialized_file = os.path.join(save_folder, "img_serialized.txt")
            #image_filename = f"frame_{timestamp}.jpg"
            #depth_filename = f"depth_{timestamp}.jpg"
            cv2.imwrite(image_filename, pframe)
