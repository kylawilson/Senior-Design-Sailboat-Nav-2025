...
xoutRgb = pipeline.create(dai.node.ColorCamera)
    ...
    with open("roi_distances.txt", "a") as file:  # Open the file in append mode
        while True:
            ...
            for depthData in spatialData:
                ...
                if distance <= 2000 and distance >= 700:
                    color = twom

                elif distance <= 700 and distance != 0:
                    color = onem
            ...
            # Show the frame
            black_screen = np.zeros_like(depthFrameColor)
            #cv2.imshow("depth", black_screen)
